...
function classColor(title: string): string {
  ...
  if (t.includes('arms') || t.includes('aaa') || t.includes('abs')) return '#7a4a2a'
  return '#a08568'
}
...
type WeekCache    = { sessions: Session[]; bookedMap: BookedMap; waitlistMap: WaitlistMap; staffMap: Record<string, string> }
...
export default function ScheduleClient({
  ...
  const [staffMap,     setStaffMap]     = useState<Record<string, string>>(initialStaffMap)
  const [bookedMap,    setBookedMap]    = useState<BookedMap>(initialBookedMap)
  const [waitlistMap,  setWaitlistMap]  = useState<WaitlistMap>(initialWaitlistMap)
  ...
  async function fetchWeekData(offset: number) {
    ...
    const cached: WeekCache = {
      sessions:    schedData.sessions ?? [],
      bookedMap:   bm,
      waitlistMap: wm,
  ...
  useEffect(() => {
    ...
    // Skip full loading state on first render if server pre-fetched initial data
    const skipLoading = first && weekOffset === 0 && hasInitial
    const hasCached   = !!weekCacheRef.current[weekOffset]

    if (!skipLoading && !hasCached) {
      setLoading(true)
      setError(false)
    }

    fetchWeekData(weekOffset)
      .catch(() => { if (!skipLoading && !hasCached) setError(true) })
      .finally(() => setLoading(false))

    const id = setInterval(() => {
      fetchWeekData(weekOffset).catch(() => {})
    }, 30_000)
    return () => clearInterval(id)
  }, [weekOffset]) // eslint-disable-line react-hooks/exhaustive-deps

  // Background prefetch of next week once current week loads
  useEffect(() => {
    if (loading || weekOffset !== 0) return
    if (weekCacheRef.current[1]) return  // already cached
    const delay = setTimeout(() => {
      fetchWeekData(1).catch(() => {})
    }, 2000) // wait 2s after current week loads
    return () => clearTimeout(delay)
  }, [loading, weekOffset]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function onTouchEnd(e: React.TouchEvent) {
    ...
    if (delta > 70 && atTop && !refreshing && !loading) {
      setRefreshing(true)
      fetchWeekData(weekOffset).finally(() => setRefreshing(false)).catch(() => {})
    }
  }
  ...
  async function handleCancel(sessionId: string, bookingId: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(d.error ?? 'Cancel failed', false)
      }
    } catch {
      showToast('Network error', false)
    } finally {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: T.linen, overflow: 'hidden' }}>
      ...
      <div
        ...
        style={{ flex: 1, overflowY: 'auto', borderTop: `1px solid ${T.rule}`, background: T.canvas }}
      >

        {/* Pull-to-refresh indicator */}
        {refreshing && (
          <div style={{ padding: '12px 0', textAlign: 'center', borderBottom: `1px solid ${T.rule}` }}>
            <div style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontSize: 10, fontWeight: 500, letterSpacing: '0.14em', textTransform: 'uppercase', color: T.muted }}>Refreshing…</div>
          </div>
        )}
        ...
        {error && !loading && (
          <div style={{ padding: '40px 20px', textAlign: 'center' }}>
            ...
            <button
              ...
              style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontSize: 10, fontWeight: 500, letterSpacing: '0.14em', textTransform: 'uppercase', color: T.brown, background: 'none', border: `1px solid ${T.brown}`, padding: '8px 20px', cursor: 'pointer' }}
            >
              Try again
            </button>
          </div>
        ...
        {loading && !error && Array.from({ length: 4 }).map((_, i) => <SkeletonCard key={i} />)}

        {!loading && !error && dayClasses.length === 0 && (
              ...
              <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                {isPast ? (
                  ...
                  <>
                    ...
                    {cutoff && (
                      <div style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", fontSize: 8.5, color: T.muted, textAlign: 'right', lineHeight: 1.3 }}>
                        by {cutoff}
                      </div>
                    )}
                  </>
