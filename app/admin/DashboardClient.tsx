  ...
  return (
    <div className="h-full overflow-y-auto bg-neutral-50">
      <div className="max-w-5xl mx-auto px-4 md:px-6 py-5 md:py-6 space-y-4 md:space-y-6">
        ...
        {/* Today's schedule + recent signups — stacked on mobile, side by side on md+ */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">

          {/* Today's classes */}
          <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
            <div className="px-4 md:px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
            ...
            {todaySessions.length === 0 ? (
              <p className="px-5 py-8 text-sm text-neutral-400">No classes today.</p>
            ) : (
              todaySessions.map((s, i) => {
                ...
                return (
                  <div
                    key={s.id}
                    className={`flex items-center gap-3 px-4 md:px-5 py-3 ${i < todaySessions.length - 1 ? 'border-b border-neutral-100' : ''}`}
                  >
                    <span className="text-[12px] text-neutral-400 w-14 shrink-0">{fmt12(s.start)}</span>
                    <span className="text-[13px] font-medium text-neutral-800 flex-1 truncate min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      <span className={`text-[12px] font-semibold w-10 text-right ${full ? 'text-red-500' : 'text-neutral-700'}`}>
          ...
          <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
            ...
            {recentSignups.length === 0 ? (
              <p className="px-5 py-8 text-sm text-neutral-400">No new clients in 30 days.</p>
            ) : (
              recentSignups.map((c, i) => {
                ...
                return (
                  <div
                    key={c.id}
                    className={`flex items-center gap-3 px-4 md:px-5 py-3 ${i < recentSignups.length - 1 ? 'border-b border-neutral-100' : ''}`}
                  >
                    <div className="w-7 h-7 rounded-full bg-neutral-900 text-white text-[11px] font-semibold flex items-center justify-center shrink-0">
                      {initials}
                    </div>
        ...
        {expiringSoon.length > 0 && (
          <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
            <div className="px-4 md:px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
              <div>
                <h3 className="text-[13px] font-semibold text-neutral-900">Expiring soon</h3>
                <p className="text-[11px] text-neutral-400 mt-0.5">Within 14 days</p>
              </div>
            ...
            <div className="divide-y divide-neutral-100">
              {expiringSoon.slice(0, 6).map(m => {
                ...
                return (
                  <div key={m.id} className="flex items-center justify-between px-4 md:px-5 py-3">
                    <div className="min-w-0 mr-3">
                      <p className="text-[13px] font-medium text-neutral-900 truncate">{name}</p>
                      <p className="text-[11px] text-neutral-400 mt-0.5 truncate">{m.planName} · Expires {fmtDate(m.endDate)}</p>
                    </div>
        ...
        {/* Quick links — 3 cols on mobile, 6 on desktop */}
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2 md:gap-3">
          {[
            ...
            <Link
              key={href}
              href={href}
              className="flex items-center justify-center h-11 bg-white border border-neutral-200 rounded-xl text-[12px] md:text-[13px] text-neutral-600 hover:border-black hover:text-black transition-colors font-medium touch-manipulation"
            >
              {label}
            </Link>
          ))}
...
function StatCard({
  ...
  return (
    <Link href={href} className="bg-white border border-neutral-200 rounded-xl px-4 md:px-5 py-4 hover:border-neutral-400 transition-colors block">
      ...
      <p className="text-[12px] text-neutral-400 mt-1">{sub}</p>
    </Link>
  )
}
