...
export default function ClassesClient({ initialServices }: { initialServices: Service[] }) {
  ...
  async function createSession() {
    ...
    // Keep modal open to add another session
    setSessionDate('')
    setSessionStartTime('')
    setSessionEndTime('')
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header */}
      <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-neutral-200 bg-white">
        <div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {services.map(service => (
          <div key={service.id} className="border-b border-neutral-100">
            {/* Service row */}
            <div
              className="flex items-center px-6 py-4 hover:bg-neutral-50 cursor-pointer transition-colors"
            ...
            >
              ...
              <div className="flex items-center gap-3 ml-4 shrink-0">
                <span className="text-[12px] text-neutral-500">
                  {service.upcomingSessions} upcoming
                </span>
                <button
                  onClick={e => { e.stopPropagation(); openAddSession(service) }}
                  className="h-7 px-3 text-[11.5px] font-medium border border-neutral-200 rounded-lg text-neutral-600 hover:border-neutral-400 hover:text-neutral-900 transition-colors"
                >
                  + Session
                </button>
                {deleteConfirmId === service.id ? (
                  <div className="flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
                    ...
                    <button
                      ...
                      className="h-6 px-2.5 text-[11px] font-semibold text-white bg-red-600 hover:bg-red-700 rounded-md disabled:opacity-40"
                    >
                      {deletingId === service.id ? '…' : 'Yes'}
                    </button>
                    <button
                      onClick={() => setDeleteConfirmId(null)}
                      className="h-6 px-2 text-[11px] text-neutral-500 hover:text-neutral-800"
                    >
                      No
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    className="h-7 w-7 flex items-center justify-center text-neutral-300 hover:text-red-500 transition-colors text-sm"
                    title="Delete class"
                  >
                    ✕
                  </button>
                )}
                <span className={`text-neutral-400 text-xs transition-transform ${expandedId === service.id ? 'rotate-180' : ''}`}>
                  ▼
                </span>
              </div>
            </div>
                    ...
                    {(sessions[service.id] ?? []).map(session => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="text-neutral-300 hover:text-red-500 transition-colors text-sm disabled:opacity-40"
